...
Общая информация о выборке:
  ...
  - Критерий "плохих": 30 или больше дней просрочки за первые 3 месяца с даты выдачи кредита

  - Количество наблюдений: `r nrow(data)`
  
  - Количество "плохих": `r sum(data$def)`
  
  - Уровень дефолта: `r round(mean(data$def), 3)`
  
Следующим шагов будет подготовка функций для последующего анализа:
...
Выбор 1 переменной для удаления из модели:
...
Теперь мы имеем модель с статистически значимыми переменными и Джини `r Gini(fit, data) %>% round(3)` и можем приступать к оценке модели.
...
# Lift
Lift <- quantile(pr,.9)/mean(data[, "def"])
Lift %>% round(3)

# Колмогоров-Смирнов
KS <- predict(fit, newdata = data, type = 'response') %>% 
  prediction(data$def) %>%
  performance('tpr', 'fpr') %>% (function(x) {
  v <- x@y.values[[1]] - x@x.values[[1]]
  v[which.max(v)]
})
KS %>% round(3)
```
...
- Джини: `r Gini(fit, data) %>% round(3)`

- Лифт (топ 10%): `r Lift %>% round(3)`

- Колгоморов-Смирнов: `r KS %>% round(3)`
